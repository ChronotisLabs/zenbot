...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function isFiat () {
      return !s.currency.match(/^BTC|ETH|XMR|USD|EUR|USDT$/)
    }
            ...
            s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  var marked_price
                  if (type === 'buy') {
                    marked_price = n(quote.bid).subtract(n(quote.bid).multiply(so.markup_pct / 100)).format(s.product.increment, Math.floor)
                    ...
                    marked_price = n(quote.ask).add(n(quote.ask).multiply(so.markup_pct / 100)).format(s.product.increment, Math.ceil)
                    if (n(order.price).value() > marked_price) {
                      msg(marked_price + ' vs our ' + order.price)
                      cancelOrder(true)
                    }
    ...
    function writeReport (is_progress, blink_off) {
      ...
      if (orig_capital) {
        ...
        var buy_hold_profit = (buy_hold - orig_capital) / orig_capital
        //process.stdout.write(' '+s.period.close +" "+orig_capital)
         process.stdout.write(' ')
        process.stdout.write(z(8, pct(buy_hold_profit), ' ')[buy_hold_profit >= 0 ? 'green' : 'red'])
         process.stdout.write(' ')
